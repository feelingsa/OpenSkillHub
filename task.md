...
- [x] M2 和 M5 分别以用户端、管理端 SVG 为视觉回归输入：在登记视口保存实现截图，记录允许差异区域，并对 1:1 壳层完成可复查的视觉验收。
...
决定：生成页面持久化在 `node/frontend/generated/`，供重启后的本机 Hub 直接复用；它们属于运行时 AI 产物，不提交 Git，版本元数据保存在 Hub 数据库中。经过人工审核、需要长期维护的页面可在后续单独纳入版本控制。
...
- [x] 为提示词增加版本号；版本变化时页面标记为 `stale`，不立即删除旧页面（通过 `HUB_PAGE_PROMPT_VERSION` 控制）。
...
- [x] 检查生成文件是否包含外部脚本、绝对路径、密钥、危险跳转或绕过共享运行时的网络请求；校验失败不激活页面，并由测试覆盖。
- [x] 用无头浏览器验证页面能够加载、关键区域存在、没有明显溢出或脚本错误。
- [x] 真实生成验收前，调用 OpenCode `GET /session` 确认新建生成会话的 `model.providerID` 和 `model.id` 与管理员选择的模型一致；仅“健康”状态不足以证明模型配置已生效。
...
- [x] 提供管理员操作：生成、重新生成、切换预设、查看生成日志、回滚上一版。
...
- [x] 下载接口只接收 `artifactId`，并再次验证用户和运行归属。
...
- [x] 提供当前单用户的运行历史与详情页：`/runs` 展示已持久化运行，`/runs/:runId` 展示事件时间线和安全产物列表；M6 接入真实身份后按 owner 隔离。
- [x] 定义产物保留时间、过期清理和管理员保留策略。
...
- [x] 明确初版身份方案。建议至少使用管理员创建的账号和密码，不允许匿名执行 Skill。
- [x] 实现登录、会话过期、退出和密码安全存储。
- [x] 定义管理员和普通用户角色。
- [x] 管理员可配置 OpenCode、扫描 Skill、生成页面、查看系统状态。
- [x] 普通用户只能浏览已启用 Skill、运行 Skill、处理自己的交互和访问自己的产物。
- [x] 所有上传、运行、日志和产物按用户隔离。
- [x] 增加并发限制、单用户运行额度、上传大小限制和请求速率限制。
- [x] 对高风险 Skill 增加管理员启用开关和额外确认。
- [x] 增加操作审计：登录、运行、权限回复、下载、配置变更和页面生成。
- [x] 设置严格 CORS，不使用 `Access-Control-Allow-Origin: *`。
- [x] 增加 CSP、安全响应头和 CSRF 防护。
- [x] 默认只监听指定局域网网卡或 `0.0.0.0`，不得自动映射公网。
- [x] 提供 Windows 防火墙配置说明，仅放行选定私有网络和端口。
- [x] 支持固定局域网 IP 或内部 DNS 名称。
- [x] 推荐使用 Caddy/Nginx 反向代理；涉及账号时启用 HTTPS。

验收标准：另一台未安装任何 Agent 或 Skill 的局域网设备，可以登录、运行一个 Skill、回答交互问题并下载产物；不能读取其他用户数据。

## 14. 阶段 10：管理页面和运维能力

- [x] 按管理端 SVG 还原公共壳层、侧边导航、表格、筛选、抽屉和确认弹窗中明确出现的布局；权限状态按阶段 0 的补充状态定义实现。
...
- [x] 管理端长列表和队列更新使用轻量 GSAP 状态动画；不得用动画隐藏错误、权限变化或任务状态。
- [x] 增加系统状态页：Node、OpenCode、数据库、扫描器、生成队列和运行队列。
- [x] 增加 Skill 管理页：启用/禁用、扫描状态、页面版本、提示词预设和最后错误。
- [x] 增加运行管理页：运行人、Skill、状态、开始时间、耗时和终止操作。
- [x] 增加存储使用统计和产物清理入口。
- [x] 支持导出不含密钥的诊断包。
- [x] 提供数据备份和恢复流程，至少覆盖数据库、配置引用和已生成页面。
- [x] 将 Node 服务和 OpenCode 配置为开机启动或受控 Windows 服务。
- [x] 增加版本信息和数据库迁移机制。
...
- [x] 单元测试：Manifest 解析、hash、字段验证、路径限制、权限判断和状态转换。
- [x] 集成测试：OpenCode 健康检查、Skill 扫描、页面生成、运行、问题回复、权限回复和终止。
- [x] 接口测试：非法 Skill ID、非法 runId、越权 artifactId、超大上传和断线重连。
- [x] 浏览器测试：首页卡片交互、Skill 路由、表单、实时状态、产物预览和下载。
- [ ] 视觉回归：桌面端与阶段 0 基线比较。
- [ ] 对 `node/source` 中的用户端和管理端 SVG 设计源按阶段 0 登记的基准视口截图并执行视觉差异检查；像素差异阈值、允许差异区域和“补充实现”豁免项必须在阶段 0 记录。
- [x] 安全测试：路径穿越、任意文件下载、跨用户访问、XSS、生成 HTML 注入和权限越权。
...
- [x] M3：通用 Skill 操作页 + 统一运行状态、问题/权限处理、SSE、安全产物下载及当前单用户运行历史已完成；M6 再接入真实用户 owner 校验、管理员持久授权和保留策略。
- [x] M4：三套页面生成提示词 + OpenCode 页面生成、校验、持久化、版本切换和兜底页；Terra 真实生成、持久化和浏览器验证通过，缺失页面由后台单并发队列继续补齐。
- [x] M5：管理端 UI + OpenCode/Skill/页面生成/运行/用户/存储管理，并完成 RBAC 和管理端 SVG 的 `1440 x 900` 1:1 壳层视觉验收。证据见 `node/docs/visual-acceptance-m5.md`。
- [x] M6：局域网认证、用户隔离、审计、防火墙/反向代理部署、恢复能力和完整测试；自动化验收覆盖 RBAC、CSRF、配额、上传/产物越权和审计，真实新设备 LAN 走查保留到阶段 11 最终验收。
- [ ] M7：旧 `server.js`/`public/` 的迁移映射、旧接口退役验证、300 Skill 性能烟测和可回滚灰度切换脚本已完成；待维护窗口中明确指定并下线旧 `server.js` 进程，再完成真实局域网走查后关闭本阶段。
