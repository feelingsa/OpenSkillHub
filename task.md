...
8. 整个项目包括前后端所有内容；`node/source/` 中用户端、管理端和配色 SVG 已明确呈现的页面区域，按登记视口进行视觉还原；未在设计导出中出现的交互、状态和响应式规则以本清单补充定义为准。Skill 个性化内容区遵循同一套设计令牌。
...
- [ ] 所有有意的页面动效通过共享动画模块实现，统一处理清理、响应式断点和 `prefers-reduced-motion`。
...
- [x] 已取得设计导出源：`node/source/Skill Web Hub — 用户端.svg`、`node/source/Skill Web Hub — 管理端.svg`、`node/source/Skill Web Hub — 配色.svg`。
- [ ] 将三份 SVG 作为本期唯一必需的视觉设计源登记并冻结；不再以“取得 Figma 链接/文件”为开工前置条件。
- [ ] 明确“1:1 还原”的范围：仅对 SVG 中可识别的用户端/管理端壳层、导航、卡片、表格、筛选、抽屉、弹窗及其明确文案、层级、色彩、间距和图标，在登记的基准视口进行视觉验收。
- [ ] SVG 未覆盖的组件变体、原型连接、加载/错误/无权限/离线/生成中状态、移动端布局和交互细节，按产品规则设计并登记为“补充实现”，不纳入设计导出像素比对。
- [ ] 为每个设计页面建立路由、状态和验收截图映射表。
- [ ] 统一记录加载中、空状态、错误、无权限、生成中、离线和移动端状态。
- [ ] 页面生成器只能生成 Skill 内容区域，用户端/管理端壳层和运行面板由代码维护。
...
- [x] 记录当前首页桌面端和移动端截图，作为样式回归基线（`node/docs/baselines/`）。
...
- [x] 记录三个 SVG 的画布尺寸、可识别页面区域、断点假设和资源依赖；当前导出尺寸为用户端/管理端 `3000x2940`，配色板 `1440x1100`（`node/docs/design-sources.md`）。
- [x] 将每份 SVG 中的页面/组件区域标注为“1:1 壳层”“共享代码组件”或“Skill 可生成内容区”，并为每项登记目标路由与基准视口（`node/docs/ui-route-map.md`）。
- [x] 对设计导出未提供的登录、加载、空数据、错误、无权限、生成中、离线和移动端状态补充交互定义，并明确为产品补充状态（`node/docs/ui-state-contract.md`）。
- [x] 建立设计源页面到实现路由的映射表，标出 1:1 壳层、允许合理响应式调整的区域，以及允许 Skill 页面生成器自定义的内容区（`node/docs/ui-route-map.md`）。
...
- [x] 对设计源中可识别的加载、空数据、错误、无权限、离线和移动端状态建立单独验收样本；设计源未覆盖的状态由产品规则补齐（`node/docs/ui-state-contract.md`）。
...
- [ ] 在 `node/` 初始化独立 Node/TypeScript 项目。
- [ ] 选择服务框架，建议 Fastify；保持依赖数量可控。
- [ ] 增加开发、构建、启动、测试和类型检查脚本。
- [ ] 将前端依赖纳入 Node 项目管理，固定 `gsap` 版本；不依赖运行时 CDN。
- [ ] 增加统一配置模块，支持环境变量和本地配置文件。
- [ ] 配置服务监听地址，默认 `0.0.0.0`，端口默认 `5177`。
- [ ] 将 `node/frontend/` 作为静态目录提供，并支持 `/skills/:skillId` 页面路由。
- [ ] 增加结构化日志和请求 ID。
- [ ] 增加全局错误处理，生产响应不得返回服务端堆栈和本机路径。
- [ ] 增加优雅退出，停止时关闭 OpenCode 连接、SSE 和运行队列。
- [ ] 创建共享 `motion` 模块，封装 GSAP 的进入、退出、列表交错、状态变化和 reduced-motion 行为。
- [ ] 所有路由切换和组件卸载都要能杀掉当前 tween/timeline，禁止页面切换后继续操作已销毁 DOM。
...
- [ ] 定义 `OpenCodeConfig`：命令路径、服务地址、端口、工作目录、启动模式和超时时间。
- [ ] 支持两种连接方式：连接已运行的 OpenCode；由 Node 启动和守护 OpenCode。
- [ ] 启动前检查命令是否存在，并记录版本和能力信息。
- [ ] 实现 OpenCode 健康检查，不能只判断端口监听。
- [ ] 实现自动启动、启动超时、异常退出检测和有限次数重启。
- [ ] 防止多个 Node 进程重复启动同一个 OpenCode 服务。
- [ ] 将 OpenCode 标准输出和错误输出写入受控日志文件。
- [ ] OpenCode 不可用时：首页仍可打开、目录仍可浏览，运行按钮显示明确不可用状态。
- [ ] 不在浏览器中暴露 OpenCode 实际地址、命令路径和服务端凭据。

验收标准：重启电脑或服务后，只启动 Node 即可恢复 OpenCode 连接；局域网用户不需要执行任何命令。

## 7. 阶段 3：扫描并标准化全部 Skill

- [ ] 优先通过 OpenCode Skill API 获取当前可用 Skill 列表。
- [ ] 对 API 返回信息不足的 Skill，在配置允许的 Skill 根目录内补充读取 `SKILL.md`、`skill.md`、`README.md`、脚本和静态资源。
- [ ] 不允许扫描配置根目录之外的任意路径。
- [ ] 定义统一的 `SkillManifest` 数据结构。
...
- [ ] 解析 YAML frontmatter 中的名称、描述和元数据。
- [ ] 解析文档中明确声明的输入、输出、默认值、枚举和必填性。
- [ ] 识别文件上传、URL、文本、数字、布尔值、选项和本地项目等输入类型。
- [ ] 解析不到明确字段时，使用通用 `taskText` 输入并记录低置信度警告。
- [ ] 输出推断只能基于明确文档证据，不能虚构文件路径。
- [ ] 修复当前生成器存在的流程未写入问题，确保解析到的 workflow 进入 Manifest。
- [ ] 计算 Skill 内容指纹，覆盖主说明、相关脚本和用于页面展示的资源。
- [ ] 将扫描结果保存到 SQLite，记录新增、更新、删除和禁用状态。
- [ ] 实现启动扫描、手动扫描和定时扫描。
- [ ] 扫描失败只影响对应 Skill，并保留上一次有效数据。
...
- [ ] 将现有首页视觉基线迁移到 `node/frontend/index.html`，保留主结构和视觉样式。
- [ ] 保留 `.skill-deck` 卡片层叠模型、滚轮切换、悬停和弹窗动画。
- [ ] 按用户端 SVG 还原公共壳层中明确出现的布局；登录态、顶部状态、错误提示、空状态和移动端布局按阶段 0 的补充状态定义实现。
- [ ] 将卡片首页拆为可复用的 `SkillDeck`、`SkillCard`、`SkillPreviewModal` 和 `ConnectionState` 组件，但保持现有 DOM 视觉结果。
- [ ] 使用共享 GSAP 模块实现卡片堆叠、切换、抬升、弹窗打开/关闭和列表进入动画；不能在每个 Skill 页面重复实现这些动效。
- [ ] 使用 `gsap.matchMedia()` 处理桌面/移动断点和 `prefers-reduced-motion`，减少动效时不能影响卡片功能。
- [ ] 动画只优先修改 transform、opacity/autoAlpha 和 CSS variables，避免用 GSAP 频繁驱动 width、height、top、left 造成布局抖动。
- [ ] 删除 `node/frontend/app.js` 中硬编码的 `skillCards`，改为请求 `GET /api/skills`。
- [ ] `stackSize` 根据实际 Skill 数量计算，不再复制同一批 Skill 填充固定 16 张卡片。
- [ ] 每张卡片展示 Skill 名称、简述、提供方、页面状态和运行可用状态。
- [ ] 为缺少预览图的 Skill 提供统一占位图，不阻塞目录展示。
- [ ] 弹窗说明改为读取 Skill Manifest，不直接从任意路径 fetch Markdown。
- [ ] 实现“开始使用”按钮，路由到 `/skills/:skillId`。
- [ ] 页面未生成时，按钮显示“正在生成”或进入通用兜底页。
- [ ] 对大量 Skill 增加搜索和分类过滤，但不破坏卡片交互的主入口。
- [ ] 支持空目录、扫描中、扫描失败、OpenCode 离线等状态。
- [ ] 保持键盘操作和移动端基本可用。
...
- [ ] 创建通用基础提示词 `prompts/skill-page-base.md`。
- [ ] 创建至少三套布局提示词：
  - [ ] `form-first.md`：以结构化参数和文件上传为核心。
  - [ ] `workflow-console.md`：以步骤、状态和长时间任务为核心。
  - [ ] `artifact-workbench.md`：以预览、文件产物和下载为核心。
- [ ] 根据 Manifest 的输入、输出和 workflow 自动选择默认预设。
- [ ] 允许管理员为单个 Skill 固定预设或重新生成。
- [ ] 提示词必须包含以下硬约束：
  - [ ] 延续 Hub 的设计语言、色彩变量和圆角尺度。
  - [ ] 不生成营销落地页，首屏直接是可操作界面。
  - [ ] 所有输入字段必须来自 Manifest，不得虚构参数。
  - [ ] 页面必须包含运行状态、问题/权限交互、日志和产物区域。
  - [ ] 不直接调用 OpenCode，不包含服务端地址和凭据。
  - [ ] 仅通过共享 `skill-runtime.js` 与 Node 通信。
  - [ ] 禁止引用未经批准的外部 CDN 和远程脚本。
  - [ ] 输出必须符合固定文件和 JSON 协议。
- [ ] 提示词不得要求 AI 重建设计 SVG 规定的 Hub 壳层；壳层、登录、运行面板、产物区和管理端 UI 由代码维护。
- [ ] 生成内容只能填充 Skill 专属的内容区域，不能覆盖全局路由、鉴权、导航、CSP 或共享运行时。
...
- [ ] 实现页面生成队列，限制同时生成数量，防止压垮 OpenCode。
- [ ] 为每个生成任务创建独立 OpenCode 会话和临时工作目录。
- [ ] 把 Skill Manifest、选定预设、共享运行时接口和允许修改的文件范围传给 OpenCode。
- [ ] 要求 OpenCode 输出：`index.html`、`styles.css`、可选 `view.js`、`view.manifest.json`。
- [ ] 先写入临时目录，完成校验后再原子替换 `node/frontend/generated/<skill-id>/`。
- [ ] 校验 HTML、CSS、JavaScript 语法和 Manifest schema。
- [ ] 检查生成文件是否包含外部脚本、绝对路径、密钥、危险跳转或绕过共享运行时的网络请求。
- [ ] 用无头浏览器验证页面能够加载、关键区域存在、没有明显溢出或脚本错误。
- [ ] 生成成功后记录 Skill hash、提示词版本、生成时间和 OpenCode 会话 ID。
- [ ] 生成失败时保留旧页面，并保存错误摘要和重试入口。
- [ ] 首次同步后自动排队生成缺失页面；不要阻塞首页启动。
- [ ] Skill 内容不变时直接复用已生成页面。
- [ ] Skill 更新时继续提供旧页面，同时后台生成新版，成功后再切换。
...
- [ ] 页面提交表单时只发送 Manifest 字段值和已上传文件的 ID。
- [ ] Node 验证字段类型、长度、必填性和允许值。
- [ ] Node 根据 Skill Manifest 构造 OpenCode 提示词，浏览器不得自行决定系统指令。
- [ ] 每次运行创建唯一 `runId`、OpenCode session ID、用户 ID 和工作目录。
- [ ] 实现开始、继续、终止和超时操作。
- [ ] 将 OpenCode 消息标准化为统一事件：
...
- [ ] 通过 SSE 将运行事件推送给对应浏览器。
- [ ] 浏览器断线重连后，可按事件序号恢复运行状态。
- [ ] 保存运行摘要和必要日志，重启服务后仍可查看历史结果。
- [ ] 运行中的 OpenCode 问题和权限请求必须绑定 `runId`，不能被其他用户回答。
- [ ] 页面提供回答问题、允许一次、始终允许、拒绝等操作。
...
- [ ] 为每次运行建立独立产物目录和允许访问的根目录集合。
- [ ] 从 OpenCode 消息、工具结果和运行目录变化中收集产物。
- [ ] 不再依赖正则从文本中提取任意本机绝对路径作为下载地址。
- [ ] 产物入库时生成 `artifactId`，保存实际路径、显示名、MIME、大小、hash、runId 和所有者。
- [ ] 下载接口只接收 `artifactId`，并再次验证用户和运行归属。
- [ ] 支持常见产物预览：图片、SVG、PDF、Markdown、HTML、文本和结构化 JSON。
- [ ] PPTX、DOCX、ZIP 等不可直接预览的文件显示类型、大小和下载操作。
- [ ] HTML/SVG 预览使用隔离 iframe 和严格 CSP，禁止访问 Hub 的认证信息。
- [ ] 产物创建后立即通过 SSE 更新页面。
- [ ] 运行结束后展示最终状态、摘要、耗时、产物数量和下载列表。
...
- [ ] 按管理端 SVG 还原公共壳层、侧边导航、表格、筛选、抽屉和确认弹窗中明确出现的布局；权限状态和响应式断点按阶段 0 的补充状态定义实现。
...
- [ ] M0：设计 SVG 源登记与解析 + 1:1 范围、页面/状态/视口映射 + 设计令牌、GSAP 版本和动效基线冻结。
- [ ] M1：Node/TypeScript 骨架 + OpenCode 健康检查、生命周期管理 + 动态 Skill 扫描和 Manifest 入库。
- [ ] M2：用户端壳层和现有卡片首页动态化；保持卡片交互，接入 GSAP 共享动画模块。
- [ ] M3：通用 Skill 操作页 + 统一运行状态、问题/权限处理、SSE 和安全产物下载。
- [ ] M4：三套页面生成提示词 + OpenCode 页面生成、校验、持久化、版本切换和兜底页。
- [ ] M5：管理端 UI + OpenCode/Skill/页面生成/运行/用户/存储管理，并完成 RBAC。
...
- [x] 用户端、管理端和配色板设计导出已放入 `node/source/`。
- [x] 本期以 `node/source/` 中三份 SVG 作为视觉验收源；无需等待原始 Figma 链接或文件。
- [ ] 若后续需要还原 SVG 未包含的组件变体、原型连接或精确响应式规则，再补充原始 Figma 文件或交互说明；这不阻塞 M0-M6。
